...
if ( !class_exists( 'Time_Log' ) ) {
    ...
    class Time_Log
    {
        ...
        private function get_time_log_from_db() {
            ...
            $sql_result = $wp_db->get_results($sql_string);
            catch_sql_errors(__FILE__, __FUNCTION__, $wp_db->last_query, $wp_db->last_error);
        ...
        private function create_sql_string() {                         
            ...
            if ( (isset($_GET['client'])) and (urldecode($_GET['client']) <>"") and (urldecode($_GET['client']) <> null) ) {
                $wherecriteria = $wp_db->prepare('WHERE tt_client.Company LIKE %s', urldecode($_GET['client']));
            }
            if ( (isset($_GET['notes'])) and (urldecode($_GET['notes']) <>"") and (urldecode($_GET['notes']) <> null) ) {
                ...
                $search_like = $wild . $wp_db->esc_like( $search_string ) . $wild;
                if ($wherecriteria == "") {
                    $wherecriteria = "WHERE ";
                } else {
                    $wherecriteria .= " AND "; 
                }
                $wherecriteria .= $wp_db->prepare('tt_time.TNotes LIKE %s', $search_like);
            }
            ...
            if ( (isset($_GET['task'])) and (urldecode($_GET['task']) <>"") and (urldecode($_GET['task']) <> null) and (urldecode($_GET['task']) <> "null") ) {
                ...
                $wherecriteria .= $wp_db->prepare('tt_task.TaskID = %s', urldecode($_GET['task']));
            }
            ...
            if ( isset($_GET['start']) and (urldecode($_GET['start']) <>"") ) {
                ...
                $wherecriteria .= $wp_db->prepare('tt_time.StartTime >= %s', $_GET['start']);
            }
            ...
            if ( isset($_GET['end'])  and (urldecode($_GET['end']) <>"") ) {
                ...
                $wherecriteria .= $wp_db->prepare('tt_time.StartTime <= %s', $_GET['end']);
            }
        ...
        private function get_html() {
            ...
            foreach ($this->open_items as $item) {
                ...
                $table .= "<td contenteditable=\"true\" onBlur=\"updateDatabase(this, 'tt_time', 'TimeID', 'StartTime'," . $item->TimeID . ")\">" . $item->StartTime . "</td>";
                $table .= "<td contenteditable=\"true\" onBlur=\"updateDatabase(this, 'tt_time', 'TimeID', 'EndTime'," . $item->TimeID . ")\">" . $item->EndTime . "</td>";
