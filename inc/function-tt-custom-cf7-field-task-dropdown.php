...
function custom_task_name_form_tag_handler( $tag ) {
  ...
  $atts = array(
    ...
    foreach ($task_list as $val) {
      $task_identifier_string = htmlspecialchars_decode($val->TaskID, ENT_QUOTES) . "-" . nl2br(stripslashes($val->TDescription));     
      if ((isset($_GET['task-name'])) AND ( $_GET['task-name'] == $task_identifier_string )) {
