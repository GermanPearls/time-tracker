    ...
    class Pending_Time
    {
        ...
        private function get_all_data_for_display($time_entries) {
            ...
            foreach ($time_entries as $i => &$item) {
                ...
                $view_task_detail_button = "<button onclick='location.href=\"" . TT_HOME . "view-task/?task-id=" . esc_attr($item["TaskID"]) . "\"' id=\"view-task-detail-" . esc_attr($item["TaskID"]) . "'\" class=\"open-task-detail-page tt-button tt-table-button\">View</button>";
                $edit_task_button = "<button onclick='location.href=\"" . TT_HOME . "edit-task/?task-id=" . esc_attr($item["TaskID"]) . "\"' id=\"edit-task-detail-" . esc_attr($item["TaskID"]) . "'\" class=\"open-task-edit-page tt-button tt-table-button\">Edit</button>";
                $item["TaskID"] = [
                    "value" => $item["TaskID"],
                    "button" => [
